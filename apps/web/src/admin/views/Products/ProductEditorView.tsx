...
export const ProductEditorView = ({ sku }: ProductEditorViewProps) => {
  ...
  const saveProduct = async () => {
    ...
    if (!compareAtResult.ok) {
      setSaving(false)
      setError(compareAtResult.error)
      return
    }
    ...
    if (categoryId != null && (!Number.isInteger(categoryId) || categoryId <= 0)) {
      setSaving(false)
      setError('Nieprawidlowa kategoria')
      return
    }
    ...
    try {
      if (isCreateMode) {
        ...
        if (!normalizedSku) {
          setError('SKU produktu jest wymagane')
          return
        }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-[#1A1A1A]">
            {isCreateMode ? 'Nowy produkt' : `Edycja produktu ${form.sku}`}
          </h1>
          <p className="text-sm text-[#737373] mt-1">
            Priorytet: poprawne dane SKU, ceny, stanu i obrazow z zapisem URL w Neon.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isCreateMode ? (
            <Link href={`/admin/products/${encodeURIComponent(sku)}/wine`} className="btn-secondary text-sm">
              Wine details
            </Link>
          ) : null}
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 bg-white rounded-xl border border-[#E5E4E1] p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs uppercase tracking-wider text-[#737373] mb-1">Allegro offer ID</label>
              <input
                className="admin-input w-full"
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button className="btn-primary text-sm disabled:opacity-40" disabled={saving || uploading} onClick={() => void saveProduct()}>
            ...
            {!isCreateMode && product?.isActive ? (
              <button
                ...
                onClick={async () => {
                  const accepted = window.confirm(`Zdezaktywowac produkt ${sku}?`)
                  if (!accepted) return
                  try {
        ...
        <div className="bg-white rounded-xl border border-[#E5E4E1] p-4 space-y-4">
          ...
          {form.imageUrl ? (
            <div className="rounded-lg border border-[#E5E4E1] p-2 bg-[#FAFAF9]">
              <img src={form.imageUrl} alt="Podglad produktu" className="w-full h-48 object-contain" />
            </div>
          ) : (
            <div className="rounded-lg border border-dashed border-[#D4D3D0] p-6 text-center text-sm text-[#A3A3A3]">
          ...
          {!isCreateMode ? (
            <button
              className="btn-secondary text-sm disabled:opacity-40 w-full"
          ...
          {!isCreateMode ? (
            <p className="text-xs text-[#737373]">
              Upload zapisuje plik w R2 pod kluczem `products/&lt;sku&gt;/main.*` i ustawia `products.image_url` w Neon.
            </p>
          ) : null}
        </div>
      </div>
