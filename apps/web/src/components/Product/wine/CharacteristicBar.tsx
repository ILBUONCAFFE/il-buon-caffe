...
export const CharacteristicBar = ({ label, value, delay = 0 }: { label: string; value: number; delay?: number }) => (
  ...
  >
    <span className="text-[13px] sm:text-sm w-20 sm:w-24 shrink-0" style={{ color: palette.textMuted }}>{label}</span>
    ...
    <span className="text-[13px] sm:text-sm font-medium w-10 text-right tabular-nums" style={{ color: palette.textSecondary }}>{value}%</span>
  </motion.div>
);
