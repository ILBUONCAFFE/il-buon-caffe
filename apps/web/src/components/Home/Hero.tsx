...
export const Hero = () => {
  ...
  // Smooth spring — spotlight follows with inertia
  const spotX = useSpring(rawX, { stiffness: 80, damping: 20, mass: 0.5 });
  const spotY = useSpring(rawY, { stiffness: 80, damping: 20, mass: 0.5 });

  // Spotlight gradient that follows the cursor
  const spotlight = useMotionTemplate`radial-gradient(520px circle at ${spotX}px ${spotY}px, rgba(163,127,91,0.13), transparent 75%)`;
  const spotlightDark = useMotionTemplate`radial-gradient(520px circle at ${spotX}px ${spotY}px, rgba(163,127,91,0.2), transparent 75%)`;

  // Subtle parallax on orbs — opposite direction to mouse
  const orbParX = useTransform(spotX, [0, 1400], [20, -20]);
  const orbParY = useTransform(spotY, [0, 900], [15, -15]);
  ...
  const handleMouseLeave = useCallback(() => {
    rawX.set(typeof window !== "undefined" ? window.innerWidth / 2 : 700);
    rawY.set(typeof window !== "undefined" ? window.innerHeight / 2 : 400);
  }, [rawX, rawY]);
  ...
  return (
    <section
      ...
      className="relative min-h-[88vh] bg-white dark:bg-brand-950 flex items-center border-b border-brand-100 dark:border-white/5 overflow-hidden"
    >
      {/* ── Background layer ── */}
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">

        {/* Ambient orbs — parallax with mouse */}
        <motion.div
          style={{ x: orbParX, y: orbParY }}
          className="absolute inset-0"
        >
          <div
            className="absolute -top-[20%] -left-[10%] w-[55vw] h-[55vw] rounded-full animate-orb-1"
            style={{ background: "radial-gradient(circle, rgba(163,127,91,0.16) 0%, transparent 70%)", filter: "blur(70px)" }}
          />
          <div
            className="absolute -bottom-[20%] -right-[10%] w-[50vw] h-[50vw] rounded-full animate-orb-2"
            style={{ background: "radial-gradient(circle, rgba(88,65,49,0.12) 0%, transparent 70%)", filter: "blur(90px)" }}
          />
          <div
            className="absolute top-[30%] left-[38%] w-[35vw] h-[35vw] rounded-full animate-orb-3"
            style={{ background: "radial-gradient(circle, rgba(184,156,125,0.10) 0%, transparent 65%)", filter: "blur(55px)" }}
          />
          {/* Dark mode stronger glows */}
          <div
            className="hidden dark:block absolute -top-[15%] -left-[5%] w-[50vw] h-[50vw] rounded-full animate-orb-1"
            style={{ background: "radial-gradient(circle, rgba(163,127,91,0.25) 0%, transparent 70%)", filter: "blur(100px)" }}
          />
          <div
            className="hidden dark:block absolute -bottom-[15%] -right-[5%] w-[45vw] h-[45vw] rounded-full animate-orb-2"
            style={{ background: "radial-gradient(circle, rgba(111,80,56,0.20) 0%, transparent 70%)", filter: "blur(120px)" }}
          />
        </motion.div>
        ...
        {/* Grain */}
        <div
          className="absolute inset-0 opacity-[0.032] dark:opacity-[0.05] mix-blend-overlay animate-grain"
          style={{
